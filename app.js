var redirect_uri = "https://RubenVaD.github.io/SpotifyWebApiESP/"; // change this your value
...
function handleCurrentlyPlayingResponse(){
    if ( this.status == 200 ){
        ...
        if (data.progress_ms != null && data.item != null) {
            let progress = data.progress_ms / data.item.duration_ms;  // between 0 and 1
            let ledsToLight = Math.floor(progress * 9);  // 0 to 9 LEDs
            sendProgressToESP(ledsToLight);
        }
    }
    else if ( this.status == 204 ){
...
function sendProgressToESP(ledCount) {
    // Replace with your ESP32's IP address
    const esp32IP = "192.168.0.139";  // <-- Change this to your ESP32 IP!

    fetch(`http://${esp32IP}/ledProgress?count=${ledCount}`)
        .then(response => response.text())
        .then(data => {
            console.log("ESP32 response:", data);
        })
        .catch(err => {
            console.error("Error sending progress to ESP32:", err);
        });
}